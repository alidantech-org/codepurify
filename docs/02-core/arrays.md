...
```hbs
{! collection.arrays.name.items !}
{! collection.arrays.name.length !}
```
...
{! item.names.* !}      // Name transformations
...
{!#each entity.fields.arrays.all.items as field!}
  {!field.index!}: {!field.names.casing.camel!}
{!/each!}
```
...
{!#each entity.fields.arrays.by_query.searchable.items as field!}
  @Column("{!field.names.casing.snake!}")
...
```hbs
{! entity.fields.arrays.all.length !}
// Output: 5

{!#if entity.fields.counts.required > 0!}
...
{!#each entity.relations.arrays.one_to_many as relation!}
  {!#each relation.target.fields.arrays.all.items as field!}
    {!relation.names.casing.pascal!}.{!field.names.casing.camel!}
  {!/each!}
{!/each!}
```
