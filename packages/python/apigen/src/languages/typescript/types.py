...
def ts_field_type(
    ...
    """Return the TypeScript type label for a field."""
    base = _ts_base_type(field, schema_by_ref)

    if _field_is_ts_nullable(field, schema_by_ref):
        return f"{base} | null"
...
def dependency_is_generated_import(kind: TemplateDependencyTargetKind) -> bool:
    """Return whether a dependency target should be imported in TypeScript."""
    return kind in {
...
def _field_is_ts_nullable(
    field: ApiField,
    schema_by_ref: dict[str, ApiSchema],
) -> bool:
    """Return whether the TypeScript field type must include null."""
    if field.nullable:
        return True
...
def _ts_base_type(
    ...
    if kind == ApiFieldKind.ARRAY.value:
        return f"Array<{_ts_array_item_type(field, schema_by_ref)}>"
    ...
    if kind == ApiFieldKind.PRIMITIVE.value:
        return _ts_primitive_type(field.type.type, field.type.format)
...
def _ts_array_item_type(
    ...
    if item_kind == ApiFieldKind.PRIMITIVE.value:
        return _ts_primitive_type(field.type.item_type, field.type.item_format)
...
def _primitive_schema_type(
    ...
    direct = _ts_primitive_type(schema.primitive_type, schema.primitive_format)
    if direct != "unknown":
        return direct
...
def _ts_primitive_type(type_name: str | None, fmt: str | None) -> str:
...
def _safe_named_or_primitive_type(type_name: str | None, fmt: str | None) -> str:
    ...
    if normalized in _OPENAPI_PRIMITIVES:
        return _ts_primitive_type(normalized, fmt)

    if not type_name:
        return "unknown"

    return _sanitize_ts_type_name(str(type_name))


def _fallback_type(field: ApiField) -> str:
    if field.type.raw_type:
        return _ts_primitive_type(field.type.raw_type, field.type.format)
...
def _sanitize_ts_type_name(value: str) -> str:
    text = value.strip()
