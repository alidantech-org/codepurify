"""TypeScript output path helpers."""
...
def resource_path_for_resource(resource: ApiResource) -> tuple[str, ...]:
    """Return the TypeScript resource folder path for a resource."""
    base_path = safe_path_parts(resource.path, fallback="")
    name_path = safe_path_parts(name_text(resource.name.kebab.o), fallback=resource.id)
...
def resource_path_for_schema(
    ...
    """Return the TypeScript resource folder path for a schema."""
...
def safe_schema_file_name(schema: ApiSchema) -> str:
    """Return a safe TypeScript schema folder/file name."""
    return safe_file_name(name_text(schema.name.kebab.o), fallback=schema.id)
