...
export class FileWriter {
   ...
   * @throws CodepurifyError if write fails
   */
  async writeGeneratedFile(input: WriteGeneratedFileInput): Promise<WriteGeneratedFileResult> {
    ...
    try {
      ...
      throw createCodepurifyError(CodepurifyErrorCode.FILE_WRITE_FAILED, 'File path escapes root directory', {
    ...
    try {
      ...
      throw createCodepurifyError(CodepurifyErrorCode.BACKUP_FAILED, 'Failed to backup file before write', {
        filePath: input.filePath,
        cause: error,
      });
    }
    ...
    try {
      ...
      throw createCodepurifyError(CodepurifyErrorCode.MANIFEST_INVALID, 'Failed to update manifest after file write', {
        filePath: input.filePath,
        cause: error,
      });
    }
   ...
   * @throws CodepurifyError if any write fails
   */
  ...
  private async writeAtomically(filePath: string, content: string): Promise<void> {
    ...
    try {
      ...
      throw createCodepurifyError(CodepurifyErrorCode.FILE_WRITE_FAILED, 'Failed to write file atomically', {
        filePath,
        tempPath,
        cause: error,
      });
